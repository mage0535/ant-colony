...
class TestKnowledgePermissionsApi(unittest.TestCase):
    def test_permissions_route_reports_leader_capabilities(self) -> None:
        ...
        with patch("src.knowledge.acl.resolve_role") as mock_role, \
             ...
             patch("src.knowledge.acl.writable_scopes", return_value=[("department", "dept-2"), ("organization", "*")]), \
             patch("src.knowledge.acl.default_write_scope", return_value=("organization", "*")), \
        ...
        self.assertEqual(result["default_write_scope"], {"owner_type": "organization", "owner_id": "*"})
