...
def _prefetch_accessible_knowledge(user_id: str, text: str, context: MessageContext) -> str:
    ...
    try:
        ...
        for item in results:
            ...
            lines.append(f"[{item.owner_type.value}] {title}\n{item.content[:1200]}")
        return "\n\n".join(lines)
...
def _build_prefetched_answer(text: str, prefetched_knowledge: str) -> str:
    if not prefetched_knowledge or not _looks_knowledge_first_query(text):
        return ""
    blocks = [part.strip() for part in prefetched_knowledge.split("\n\n") if part.strip()]
    if not blocks:
        return ""
