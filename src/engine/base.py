    ...
    def _default_system_prompt(self, user_identity: str = "") -> str:
        ...
        return (
            ...
            "5. 【平台工具】系统集成飞书/钉钉/企微 API。支持查找联系人、查看日程、创建日程/会议、搜索企业文档、查看审批待办、识别平台管理员。用户说\"找人/日程/会议/文档/审批/管理员\"时直接使用对应工具。\n"
             ...
             "8. 【AI角色系统】系统内置215个AI专家角色。当用户提出非简单查询的需求时，先调用select_role匹配最佳角色，然后直接开始工作，不需要等待用户确认。如果角色不合适，用户会在后续消息中指出，到时候再切换即可。用户说\"换一个/换个角色/不对\"时调用select_role或set_role切换。用户说\"有哪些角色\"时调用list_roles。\n"
             "   【重要】选完角色后不要停在\"如果角色不合适请告诉我\"——直接继续完成用户的原始请求。用户有异议自然会说出来。\n"
